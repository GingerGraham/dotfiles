...
# and signing with "No pinentry". Common gap on minimal/WSL images. Installs a
# terminal-capable frontend (curses/tty) — no GUI/display dependency.
    ...
    case "${DOTFILES_OS}" in
        ...
        Linux)
            case "${DOTFILES_DISTRO}" in
                ...
                *) log_error "pinentry: unknown distro (${DOTFILES_DISTRO}) — install manually"; return 1 ;;
            esac
            ;;
...
# badly on Debian/Ubuntu, nvim-config (the companion config repo deployed by
# sync-external) requires Neovim 0.10+ and only selects the treesitter `main`
# branch at 0.12+, and a single install path across every platform is far
# easier to reason about than a version-floor matrix with per-distro sticky
# method tracking.
#
# Config deployment is handled entirely by sync-external (link_tree mode
# against the nvim-config companion repo — see docs/external-sync.md); this
# function owns only the binary.
...
# The retired Ansible role's `appimage` method wrote a generated bash wrapper
# script at exactly this path, and the new install wants that path for its
# symlink — so a non-symlink file there must be detected and moved aside
# rather than assuming the path is free. The content grep is purely to log a
# more useful message; the file is backed up either way.
_neovim_cleanup_wrapper_script() {
    local nvim_bin="${HOME}/.local/bin/nvim"
    [[ -e "${nvim_bin}" && ! -L "${nvim_bin}" ]] || return 0

    if grep -q 'appimage\|Wrapper generated by dotfiles' "${nvim_bin}" 2>/dev/null; then
        log_info "Found the retired Ansible nvim wrapper script at ${nvim_bin}"
    else
        log_info "Found an existing non-symlink file at ${nvim_bin}"
    fi
    local backup="${nvim_bin}.pre-dotfiles.bak"
    mv "${nvim_bin}" "${backup}"
    log_info "Backed up to ${backup}"
}

# Cleanup case 2: ~/.local/share/nvim/nvim.appimage left behind by the retired
# Ansible role.
#
# ~/.local/share/nvim is Neovim's XDG *DATA* directory — it holds the
# lazy.nvim plugin tree and site files. REMOVE THE FILE ONLY. Removing the
# DIRECTORY would destroy the plugin install. This is the single most
# dangerous line in this file.
...
install-neovim() {
    ...
    if [[ "${DOTFILES_OS}" == "Mac" ]] && command -v xattr &>/dev/null; then
    ...
    echo
    echo "  Config is managed separately by sync-external (nvim-config companion repo)."
    echo "  See docs/external-sync.md if ~/.config/nvim looks empty on a fresh machine."
}
